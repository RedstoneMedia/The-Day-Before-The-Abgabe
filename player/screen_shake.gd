...
func _new_shake():
	...
	shake_tween.interpolate_property(camera, "offset", camera.offset, rand, frequency_wait_time, TRANS, EASE)
	shake_tween.start()


func _reset():
	var frequency_wait_time = frequency_timer.wait_time
	shake_tween.interpolate_property(camera, "offset", camera.offset, Vector2(), frequency_wait_time, TRANS, EASE)
	shake_tween.start()
	priority = 0
